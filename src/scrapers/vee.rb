...
MEMBER_LIST_URL = 'https://vee-official.jp/talent/'
...
def get_streamer_list(url)
  ...
  doc.css('.p-talent__list-item a').reject do |a|
    a.parent.parent.parent['class'] == 'l-graduation'
  end.map do |a|
    a['href']
  end
end
...
def get_streamer_detail(url)
  ...
  name = doc.at_css('.p-talent_info__name-text').text.strip
  urls = doc.css('.p-talent_info_sns__list-item a').map { |a| a[:href] }.reject(&:empty?) # なんかURLが空のがある

  { name: name, links: create_link_map(urls), tags: [GROUP_NAME].uniq, page: url }
end
